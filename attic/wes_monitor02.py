...
###############################################################################
# These imports are so that we can connect to the same db as the wes_monitor_API.py
###############################################################################
...
def main():
    ...
    #Start API
    print("Launching WES monitor API on port %s" % options.port)
    #NOTE: key is close_fds
    proc = subprocess.Popen(["./wes_monitor_API.py", "-p", options.port])
    #                         shell=True, stdin=None, stdout=None, stderr=None,
    #                         close_fds=True)

    #ref: https://codehunter.cc/a/flask/how-can-i-start-a-subprocess-in-python-that-runs-independently-and-continues-running-if-the-main-process-is-closed
    #with open("wes_monitor_API.log", "a") as log_file:
    #    subprocess.call("./wes_monitor_API.py -p %s" % options.port,
    ...
    #    os.system("nohup ./wes_monitor_API.py -p %s >> wes_monitor_API.log &" % options.port)
