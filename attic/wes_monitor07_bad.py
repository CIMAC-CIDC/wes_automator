...
class Update(Resource):
    def put(self, instance_name):
        ...
        with db_manager.ManagedSession() as session:
            wes_run = session.query(WesRun).filter(WesRun.instance_name == instance_name).first()
            #request.form works, but request.args doesn't!
            if wes_run:
                print(request)
                #print(dir(request))
...
class WesRun(db.Model):
    ...
    cores = db.Column(db.Integer, nullable=False, default=0)  #how many steps have completed
...
def getRunningCores(session):
    ...
    running = session.query(WesRun).filter(WesRun == 'RUNNING')
    sum_cores = 0
    for run in running:
        sum_cores += run.cores
    return sum_cores
...
def main():
    ...
    for wes_run in wes_runs_configs:
        ...
        result = WesRun.query.filter_by(instance_name=tmp.instance_name).first()
