...
#TRY: see if db concurrency issue can be solved by switching to mysql
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:foobar@localhost/wes_monitor'

#app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///%s' % _db_filename
#from sqlalchemy.pool import StaticPool
#app.config['SQLALCHEMY_ENGINE_OPTIONS'] = {'connect_args': {'check_same_thread':False}, 'poolclass': StaticPool}

#app.config['SQLALCHEMY_ENGINE_OPTIONS'] = {'pool_size': 10, 'poolclass': QueuePool}
#db = SQLAlchemy(app, session_options={'autocommit': True})
from sqlalchemy.orm import scoped_session
from sqlalchemy.orm import sessionmaker

db = SQLAlchemy(app,engine_options={'pool_size':10,'poolclass':QueuePool})
...
def main():
    ...
    while True:
        ...
        #print elapsed time
        now = time.time()
        print("Elapsed time %s" % time_convert(now - start_time))
        #print the runs
        for run in WesRun.query.all():
            print(run)
        print("*" *79)
        session = Session()
        foo = session.query(WesRun).all()
        #print(WesRun.query.filter_by(instance_name='wes-auto-c3rxbpckm-01').first())
        print(foo)
